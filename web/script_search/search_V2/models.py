...
class Script(models.Model):
    ...
    class Meta:
        indexes = [BTreeIndex(fields=["year"]), BTreeIndex(
            fields=["script_type"]), GinIndex(fields=["search_content"])]
        ordering = ['title']
...
def format_script_title(script_obj):
    ...
    if script_obj:
        if script_obj.script_type == 'T':
            title = '{show} (Season {season}, Episode \"{episode}\", {year})'.format(show=script_obj.title, season=str(
                script_obj.season), episode=script_obj.episode_title, year=str(script_obj.year))
        elif script_obj.script_type == 'M':
            title = '{show} ({year})'.format(
                show=script_obj.title, year=str(script_obj.year))
    else:
        title = ''
